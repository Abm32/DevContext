...
interface UpgradePromptProps {
  feature: string
  description: string
  inline?: boolean     // true = compact inline banner; false = overlay modal
  onDismiss?: () => void
}
...
export function UpgradePrompt({ feature, description, inline = false, onDismiss }: UpgradePromptProps) {
    ...
    return (
      ...
      >
        ...
        <a
          ...
          className="shrink-0 px-3 py-1.5 rounded-lg text-[11px] font-semibold text-white transition-all"
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full gap-4 p-6 text-center">
      ...
      <p className="text-[11px]" style={{ color: "rgba(255,255,255,0.2)" }}>Unlimited AI · Compare repos · Standups · Workspaces</p>
    </div>
  )
}
...
export function UsageChip({ used, limit, exhausted }: UsageChipProps) {
  return (
    ...
    >
      ...
      {exhausted && <span className="font-semibold">· Upgrade for more</span>}
    </div>
  )
}
